...
#include "Group44VOOAcutal_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Group44VOOAcutal_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Group44VOOAcutal_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Group44VOOAcutal_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Group44VOOAcutal_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Group44VOOAcutal_M, 0);

  /* Initialize model */
  Group44VOOAcutal_initialize();
