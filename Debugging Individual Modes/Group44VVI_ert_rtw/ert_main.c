...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Group44VVI_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Group44VVI_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Group44VVI_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Group44VVI_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Group44VVI_M, 0);

  /* Initialize model */
  Group44VVI_initialize();
