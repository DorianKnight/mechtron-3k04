...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Group44AOO_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Group44AOO_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Group44AOO_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Group44AOO_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Group44AOO_M, 0);

  /* Initialize model */
  Group44AOO_initialize();
